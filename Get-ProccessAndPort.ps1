...
$programs = (Get-NetTCPConnection -LocalAddress $ip | Where-Object {($_.State -eq "Listen")}) ### Listen or Established
...
foreach ($program in $programs) {
    ...
    $process = (Get-Process -PID $program.OwningProcess).Name
    $localPort = $program.LocalPort
    $remoteAddress = $program.RemoteAddress

    Write-Host "Name --> " $process ";Port --> " $localPort ";Remote address --> " $remoteAddress
